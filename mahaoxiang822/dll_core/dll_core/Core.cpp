...
bool Core::has_loop() {
	...
	while (!indegree_zero_id.empty()) {
		...
		for (int i = 0; i < word_num; i++)
			if (in_degree[i] == 0 && find(stack.begin(), stack.end(), i)== stack.end())
				indegree_zero_id.push_back(i);
	}
	while (!stack.empty()) {
...
int string2array(vector<string> string_list, char *words[]) {//返回长度
	...
	for (int i = 0; i < size; i++) {
		...
		strcpy(words[i], string_list[i].c_str());
	}
	return size;
}
...
int Core::gen_chain_word(char *words[], int len, char *result[], char head, char tail, bool enable_loop) {
	...
	vector<string> res;
	int len = max_chain_word(array2string(words, len), res, head, tail, enable_loop);
...
int Core::gen_chain_char(char* words[], int len, char* result[], char head, char tail, bool enable_loop) {
	...
	vector<string> res;
	int len = max_chain_char(array2string(words, len), res , head, tail, enable_loop);
